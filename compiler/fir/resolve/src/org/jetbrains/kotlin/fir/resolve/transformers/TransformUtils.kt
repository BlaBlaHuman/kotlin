...
internal fun FirValueParameter.transformVarargTypeToUnderlyingType() {
    if (isVararg) {
        this.transformTypeToUnderlyingVarargType()
    }
}

internal fun FirCallableDeclaration.transformTypeToUnderlyingVarargType() {
    ...
    if (returnTypeRef.delegatedTypeRef is FirResolvedTypeRef &&
        returnTypeRef.delegatedTypeRef?.source?.kind == KtFakeSourceElementKind.CollectionTypeFromVarargParameter
    ) return
    ...
    replaceReturnTypeRef(
        buildResolvedTypeRef {
            ...
            type = ConeKotlinTypeProjectionOut(returnType).createVarargUnderlyingType()
            annotations += returnTypeRef.annotations
            // ? do we really need replacing source of nested delegatedTypeRef ?
            delegatedTypeRef = returnTypeRef.copyWithNewSourceKind(KtFakeSourceElementKind.CollectionTypeFromVarargParameter)
        }
    )
}
